    ...
    <v-card class="p-3">
      <v-container>
        ...
        <v-col cols="12" sm="12" md="12" lg="4" xl="4" class="pl-0 pb-0 pr-0">
          <v-text-field class="mt-3" dense label="Buscar" type="text" v-model="options.search"></v-text-field>
        </v-col>
      </v-container>
      <v-data-table :search="options.search" :headers="headers" :items="recordsFiltered" :options.sync="options"
        :loading="loading" item-key="id" sort-by="name" hide-default-footer>
        ...
        <v-card-text>
          <v-container>
            ...
            <v-row class="pt-3" v-if="selectedTab == 0">
              <v-container>
                <v-col cols="12" sm="12" md="6" lg="4" xl="4">
                  <base-input label="Nombre" v-model="$v.editedItem.name.$model" :validation="$v.editedItem.name"
            ...
            <v-row class="pt-3" v-if="selectedTab == 1">
              <permission :editedIndex="editedIndex" :permissionProps="editedItem.permissions"
